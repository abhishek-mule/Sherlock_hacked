...
export function EnhancedStudentCard({ student, onOsintLookup }: EnhancedStudentCardProps) {
  ...
  return (
    ...
    >
      <Card className="w-full overflow-hidden border border-slate-200 dark:border-slate-700 shadow-lg hover:shadow-xl transition-all duration-300 bg-white dark:bg-slate-900">
        <div className="absolute top-0 left-0 right-0 h-24 bg-gradient-to-r from-teal-500 to-cyan-500">
          <motion.div 
            className="absolute inset-0 bg-gradient-to-r from-teal-500 to-cyan-500"
            animate={{ 
              background: ['linear-gradient(to right, #0d9488, #06b6d4)', 
                         'linear-gradient(to right, #0891b2, #14b8a6)']
            }}
        ...
        <CardHeader className="relative pt-20 pb-6 flex flex-col items-center">
          ...
          <motion.h3 
            className="mt-4 text-2xl font-bold text-slate-900 dark:text-white text-center"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
